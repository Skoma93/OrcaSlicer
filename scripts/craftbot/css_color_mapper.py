...
class CssColorMapperApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("CSS Color Mapper")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top_frame, text="Scan CSS", command=self.scan_folder).grid(row=0, column=3, padx=(8, 0), pady=4)
        ...
        self.status_var = tk.StringVar(value="Select a folder and scan for CSS colors.")
    ...
    def choose_folder(self) -> None:
        selected = filedialog.askdirectory(title="Select CSS root folder")
        if selected:
    ...
    def scan_folder(self) -> None:
        ...
        self.clear_rows()
        self.css_files = sorted(folder.rglob("*.css"))

        if not self.css_files:
            self.status_var.set("No .css files found.")
            messagebox.showinfo("No CSS files", "No .css files were found in the selected folder.")
            return

        for css_file in self.css_files:
            self._scan_single_file(css_file)

        if not self.color_counts:
            self.status_var.set(f"Scanned {len(self.css_files)} CSS files, but no supported colors were found.")
            messagebox.showinfo("No colors found", "No supported color values were found in the CSS files.")
            return
        ...
        self.status_var.set(
            f"Found {len(self.color_counts)} unique colors across {len(self.css_files)} CSS files."
        )

    def _scan_single_file(self, css_file: Path) -> None:
        try:
            ...
            try:
                ...
                text = css_file.read_text(encoding="latin-1")
        except OSError:
            return
    ...
    def apply_changes(self) -> None:
        ...
        for css_file in self.css_files:
            file_changed, replacements = self._apply_to_file(css_file, mapping)
            if file_changed:
                changed_files += 1
                changed_occurrences += replacements

        self.status_var.set(
            f"Updated {changed_occurrences} color occurrences in {changed_files} CSS files."
        )
        messagebox.showinfo(
            "Done",
            f"Updated {changed_occurrences} color occurrences in {changed_files} CSS files.",
    ...
    def _apply_to_file(self, css_file: Path, mapping: dict[str, str]) -> tuple[bool, int]:
        try:
            text = css_file.read_text(encoding="utf-8")
            encoding = "utf-8"
        except UnicodeDecodeError:
        ...
        try:
            if self.backup_var.get():
                backup_path = css_file.with_suffix(css_file.suffix + ".bak")
                if not backup_path.exists():
                    shutil.copy2(css_file, backup_path)
            css_file.write_text(new_text, encoding=encoding)
        except OSError:
            return (False, 0)
