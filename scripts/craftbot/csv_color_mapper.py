...
class CsvColorMapperApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("CSV Color Mapper")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top_frame, text="Scan CSV files", command=self.scan_folder).grid(row=0, column=3, padx=(0, 8), pady=4)
        ...
        self.summary_label = ttk.Label(bottom_frame, text="Select a folder and scan for CSV colors.")
    ...
    def select_folder(self) -> None:
        selected = filedialog.askdirectory(title="Select CSV root folder")
        if selected:
    ...
    def scan_folder(self) -> None:
        ...
        self.csv_files = sorted(folder.rglob("*.csv"))

        if not self.csv_files:
            self.summary_label.config(text="No CSV files were found.")
            self.log("No .csv files found in the selected folder or subfolders.")
            return
        ...
        for csv_file in self.csv_files:
            try:
                ...
                self.log(f"Skipped {csv_file}: {exc}")
                continue
        ...
        if not self.color_counts:
            self.summary_label.config(text=f"{len(self.csv_files)} CSV files found, but no supported colors were detected.")
        ...
        self.summary_label.config(
            text=f"Found {len(self.color_counts)} unique colors in {len(self.csv_files)} CSV files."
        )
        self.log(f"Found {len(self.color_counts)} unique colors in {len(self.csv_files)} files.")
    ...
    def apply_changes(self) -> None:
        ...
        should_continue = messagebox.askyesno(
            "Apply changes",
            f"This will update colors in {len(self.csv_files)} CSV files. Continue?",
        )
        ...
        for csv_file in self.csv_files:
            try:
                original_text = csv_file.read_text(encoding="utf-8-sig", errors="replace")
            except OSError as exc:
                self.log(f"Skipped {csv_file}: {exc}")
                continue
            ...
            try:
                ...
                csv_file.write_text(updated_text, encoding="utf-8", newline="")
            except OSError as exc:
                self.log(f"Failed to write {csv_file}: {exc}")
                continue

            files_changed += 1
            total_replacements += replacements_in_file
            self.log(f"Updated {csv_file} ({replacements_in_file} replacements)")
