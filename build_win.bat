...
REM install(TARGETS OrcaSlicerForCraftbot) carries no OPTIONAL, so installing a tree
REM whose executable was never built fails. Say so before the build starts.
if "%build_slicer%" == "ON" if "%install_slicer%" == "ON" if not "%slicer_target%" == "" if /I not "%slicer_target%" == "OrcaSlicerForCraftbot" (
...
if "%pack_deps%" == "ON" (
    ...
    echo Packing the dependencies: OrcaSlicerForCraftbot_dep_win-!dep_variant!_!build_date!.zip
    ...
    call :print_and_run !zipper! OrcaSlicerForCraftbot_dep_win-!dep_variant!_!build_date!.zip OrcaSlicerForCraftbot_dep
    %error_check%
    REM endlocal is about to discard the name, so carry the path out with it.
    for %%z in ("!DEP_TREE_PACK!\OrcaSlicerForCraftbot_dep_win-!dep_variant!_!build_date!.zip") do endlocal & set "bundle=%%~fz"
)
...
if "%build_slicer%" == "ON" (
    ...
    if not "%dry_run%" == "ON" if not "%no_configure%" == "ON" if not "%build_deps%" == "ON" if not exist "!DEP_TREE!\OrcaSlicerForCraftbot_dep\usr\local\" (
...
:summary
    ...
    set "slicer_exe=%build_dir%\src\%build_type%\orca-slicer-for-craftbot.exe"
    if "%install_slicer%" == "ON" set "slicer_exe=%build_dir%\OrcaSlicerForCraftbot\orca-slicer-for-craftbot.exe"
    for %%p in ("!slicer_exe!") do set "slicer_full=%%~fp"
    REM The 2026 generator writes OrcaSlicerForCraftbot.slnx, the releases before it
    REM OrcaSlicerForCraftbot.sln. A file already there wins, in case an older CMake
    REM configured the build.
    set "solution=OrcaSlicerForCraftbot.sln"
    if "%vs_version%" == "2026" set "solution=OrcaSlicerForCraftbot.slnx"
    if exist "!build_full!\OrcaSlicerForCraftbot.sln" set "solution=OrcaSlicerForCraftbot.sln"
    if exist "!build_full!\OrcaSlicerForCraftbot.slnx" set "solution=OrcaSlicerForCraftbot.slnx"
    ...
    if /I "%slicer_target%" == "OrcaSlicerForCraftbot" set "linked=ON"
    ...
    if "%build_slicer%" == "ON" if "%linked%" == "ON" echo   OrcaSlicerForCraftbot !slicer_full!
