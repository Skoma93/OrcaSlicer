    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour("#7D0096"));
    ...
    m_link_edit_nozzle->SetForegroundColour("#7D0096");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(125, 0, 150));
...
{
    if (enable) {
        m_ams_backup_tip->SetForegroundColour(StateColor::darkModeColorFor("#7D0096"));
    }
    else {
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#7D0096") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
