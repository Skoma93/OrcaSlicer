...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(146, 39, 143));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(146, 39, 143)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(114, 29, 111), StateColor::Pressed), std::pair<wxColour, int>(wxColour(169, 60, 166), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(146, 39, 143), StateColor::Normal));
