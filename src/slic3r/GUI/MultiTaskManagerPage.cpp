    ...
    m_task_type(type)
{
    ...
    auto m_btn_bg_enable = StateColor(
        std::pair<wxColour, int>(wxColour(114, 0, 137), StateColor::Pressed),
        std::pair<wxColour, int>(wxColour(145, 38, 166), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(125, 0, 150), StateColor::Normal)
    );
        ...
        if (get_obj()) {
            if (state_cloud_task == 0 && m_job_id == get_obj()->profile_id_) {
                ...
                if (state_device == 0) {
                    ...
                    dc.SetTextForeground(wxColour(125, 0, 150));
                    DrawTextWithEllipsis(dc, get_state_device(), FromDIP(DEVICE_LEFT_PRO_INFO), left);
                }
                else if (state_device == 2)
                {
                    ...
                    if (obj_->get_curr_stage() == _L("Printing") && obj_->subtask_) {
                        ...
                        dc.SetPen(wxPen(wxColour(125, 0, 150)));
                        dc.SetBrush(wxBrush(wxColour(125, 0, 150)));
