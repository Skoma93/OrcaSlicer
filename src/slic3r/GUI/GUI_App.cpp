...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #7D0096  -- 73
        if (bg_col != orig_col) {
...
void GUI_App::load_current_presets(bool active_preset_combox/*= false*/, bool check_printer_presets_ /*= true*/)
{
    ...
    if (printer_technology == ptFFF && !edited_printer_preset.config.opt_bool("single_extruder_multi_material")) {
        ...
        if (nozzle_diameter) {
            ...
            const size_t physical_filaments = single_filament_idex ? 1 : nozzle_diameter->values.size();
            const size_t target = physical_filaments + preset_bundle->num_mixed_filaments();
